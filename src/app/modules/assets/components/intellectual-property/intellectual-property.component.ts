...
this._route.navigate([this.forwardRouteLink]); 
  ...
  shareDataHandler({ shareData, id }) {
    ...
    if (myItem !== -1) {
      ...
      const shareMemberIdNew = sharesMemberId.map((el) => {
        if (el?.member === id) {
          return { member: id, share: sharesObj[0].share,type:'intellectualProperty' };
        }
        return el;
      });
  ...
  addSharesMember(id) {
    ...
    let sharesMemberId: Array<any> =  this.GiftBenificiary;
    this.GiftBenificiary=shareItemsHandler(sharesObj, id, sharesMemberId,'intellectualProperty'),
    this.addColorArray();
 
  }
  ngOnInit(): void {
 this._willServices.assetsBeneficiary.subscribe((assetsBeneficiary) => {
      this.GiftBenificiary=assetsBeneficiary.filter((el)=>el.type==='intellectualProperty');
      this.addColorArray();
